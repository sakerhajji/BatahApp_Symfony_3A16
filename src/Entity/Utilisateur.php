...
class Utilisateur
{
    ...
    public function removeProduit(Produits $produit): self
    {
     ...
     /**
     * @ORM\OneToMany(targetEntity=ReservationEnchere::class, mappedBy="idUser")
     */
    ...
    {
        if (!$this->reservationenchere->contains($reservation_enchere)) {
            $this->reservation_enchere[] = $reservation_enchere;
            $reservation_enchere->setIdUser($this);
        }

        return $this;
    }

    public function removeReservationEnchere(ReservationEnchere $reservation_enchere): self
    {
        if ($this->reservation_enchere->removeElement($reservation_enchere)) {
            // set the owning side to null (unless already changed)
            if ($reservation_enchere->getIdUser() === $this) {
